...
LifeSimState _state({
  ...
  LifeStats stats = const LifeStats(health: 75, energy: 75, stress: 25, relationships: 70),
...
DebriefModel _debrief({
  ...
  String counterfactual = 'Nu ai lăsat nicio penalizare evitabilă, cash-flow curat.',
...
void main() {
  group('Determinism, aceleași intrări → aceeași ieșire', () {
    test('commentOnChoice e determinist', () {
      ...
      final event = _event([const LifeChoice(label: 'A', effects: [], debrief: 'Compromis test')]);
      final a = commentOnChoice(event: event, choiceIdx: 0, before: before, after: after, seed: 42);
      final b = commentOnChoice(event: event, choiceIdx: 0, before: before, after: after, seed: 42);
  ...
  group('commentOnChoice, mai multe (trade-off + calendar/cost de oportunitate)', () {
    test('more[0] e chiar debrief-ul alegerii, când există', () {
      final before = _state(cash: const Money(100000));
      final after = _state(cash: const Money(90000));
      final event = _event([const LifeChoice(label: 'A', effects: [], debrief: 'Compromisul exact al alegerii.')]);
      final c = commentOnChoice(event: event, choiceIdx: 0, before: before, after: after, seed: 1);
    ...
    test('more[1]: cost de oportunitate corect când ≥2 zile de mâncare', () {
      ...
      final event = _event([const LifeChoice(label: 'A', effects: [], debrief: 'd')]);
      final c = commentOnChoice(event: event, choiceIdx: 0, before: before, after: after, seed: 5);
    ...
    test('more[1]: sub 2 zile → replică de umplutură (fără „zile")', () {
      ...
      final event = _event([const LifeChoice(label: 'A', effects: [], debrief: 'd')]);
      final c = commentOnChoice(event: event, choiceIdx: 0, before: before, after: after, seed: 2);
      expect(c.more[1], isNot(contains('zile')));
    });

    test('more[1]: consecință programată → quip de calendar, nu cost de oportunitate', () {
      final before = _state(cash: const Money(100000), scheduledEvents: const []);
      final after = _state(cash: const Money(100000), scheduledEvents: const [(10, 'ev_viitor')]);
      final event = _event([const LifeChoice(label: 'A', effects: [], debrief: 'd')]);
      final c = commentOnChoice(event: event, choiceIdx: 0, before: before, after: after, seed: 6);
      expect(c.more[1], isNot(contains('zile de mâncare')));
    });
  });

  group('Derivarea mood-ului (spec exact)', () {
    LifeChoice choice() => const LifeChoice(label: 'A', effects: [], debrief: 'd');
    ...
    test('cash negativ după alegere → worried', () {
      ...
      final c = commentOnChoice(
          event: _event([choice()]), choiceIdx: 0, before: before, after: after, seed: 1);
    ...
    test('stres > 70 → worried chiar cu cash pozitiv', () {
      final before = _state(
          ...
          stats: const LifeStats(health: 75, energy: 75, stress: 50, relationships: 70));
      final after = _state(
          cash: const Money(100000),
          stats: const LifeStats(health: 75, energy: 75, stress: 75, relationships: 70));
      final c = commentOnChoice(
          event: _event([choice()]), choiceIdx: 0, before: before, after: after, seed: 1);
      expect(c.mood, CashyMoodGame.worried);
    });
    ...
    test('progres la obiectiv → celebrate, chiar fără mișcare mare de cash', () {
      final before = _state(cash: const Money(10000), goalSavings: Money.zero);
      final after = _state(cash: const Money(10000), goalSavings: const Money(5000));
      final c = commentOnChoice(
          event: _event([choice()]), choiceIdx: 0, before: before, after: after, seed: 1);
      expect(c.mood, CashyMoodGame.celebrate);
    });
    ...
    test('consecință programată, fără altceva notabil → thinking', () {
      final before = _state(cash: const Money(10000));
      final after = _state(cash: const Money(10000), scheduledEvents: const [(10, 'ev_x')]);
      final c = commentOnChoice(
          event: _event([choice()]), choiceIdx: 0, before: before, after: after, seed: 1);
    ...
    test('commentOnQuietDay: cash negativ sau stres mare → worried, altfel happy', () {
      ...
      expect(commentOnQuietDay(s: broke, seed: 1).mood, CashyMoodGame.worried);
      final stressed = _state(
          cash: const Money(1000),
          stats: const LifeStats(health: 75, energy: 75, stress: 80, relationships: 70));
      expect(commentOnQuietDay(s: stressed, seed: 1).mood, CashyMoodGame.worried);
    });
  });
  ...
  group('narrateReport, segmente + cifre din debrief/stare', () {
    test('cu decizii eficiente/riscante → 7 segmente (în intervalul 5-8)', () {
      const efficient = [
        DebriefDecision(
            day: 3, eventId: 'e1', title: 'Oferta', choiceIdx: 0,
            immediate: Money(30000), delayed: Money.zero),
        DebriefDecision(
            day: 9, eventId: 'e2', title: 'Bonus', choiceIdx: 0,
            immediate: Money(50000), delayed: Money.zero),
      ];
      const risky = [
        DebriefDecision(
            day: 12, eventId: 'e3', title: 'Împrumut riscant', choiceIdx: 1,
            immediate: Money(-20000), delayed: Money(-40000)),
      ];
      final d = _debrief(
    ...
    test('fără decizii notabile → segment „constant" ține totalul în interval', () {
      final d = _debrief();
      final s = _state(day: 30);
    ...
    test('cifrele interpolate corespund EXACT stării/debrief-ului', () {
      const efficientDecision = DebriefDecision(
          day: 7, eventId: 'e1', title: 'Vânzare rapidă', choiceIdx: 0,
          immediate: Money(45000), delayed: Money.zero);
      const riskyDecision = DebriefDecision(
          day: 14, eventId: 'e2', title: 'Pariu prostesc', choiceIdx: 1,
          immediate: Money(-10000), delayed: Money(-30000));
      final d = _debrief(
      ...
      const stats = LifeStats(health: 82, energy: 60, stress: 45, relationships: 91);
