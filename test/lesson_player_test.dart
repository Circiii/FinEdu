...
void main() {
  ...
  group('fluxul playerului (u1-banii-tai: guess → blocuri+check → scenariu → pairs)',
      () {
    ...
    testWidgets('gate-urile țin până e răspunsă fiecare pagină',
      ...
      await tester.pumpWidget(_app(
          const LessonPlayerScreen(lessonId: 'u1-banii-tai')));
      ...
      await tester.tap(find.text('Continuă'));
      await tester.pumpAndSettle();
      expect(page(), 1, reason: 'gate-ul ține până sunt dezvăluite blocurile');
  ...
  group('interactives', () {
    testWidgets('swipe: wrong answers re-queue, finishing opens the gate',
      ...
      await tester.pumpWidget(_app(buildInteractive(it,
          onDone: (d) => done = d, onResult: (r) => firstTry = r)));

      // Wrong: 'Pastă de dinți' is a need, answer 'Dorință' (right bucket).
      await tester.tap(find.text('Dorință'));
      await tester.pumpAndSettle();
      expect(find.text('Igiena e nevoie.'), findsOneWidget);
      expect(done, isFalse);

      // The queue is now [Skin nou, Pastă de dinți], answer both correctly.
      await tester.tap(find.text('Dorință'));
      await tester.pumpAndSettle();
    ...
    testWidgets('cloze: wrong chip disables, correct chip fills and explains',
      ...
      await tester
          .pumpWidget(_app(buildInteractive(it, onDone: (d) => done = d)));
