...
void main() {
  group('Money, aritmetică pe int bani (niciun double)', () {
    ...
    test('egalitate structurală', () {
      ...
      expect(const Money(1).hashCode, const Money(1).hashCode); // dedup în set-uri
    });
  });
  ...
  group('LifeSimRng, SplitMix64 determinist', () {
    ...
    test('vector de aur: primele 5 nextU64 pentru seed 42', () {
      final r = LifeSimRng(42);
      final out = [for (var i = 0; i < 5; i++) r.nextU64()];
    ...
    test('seed diferit → secvență diferită', () {
      final a = [for (var i = 0; i < 5; i++) LifeSimRng(1).nextU64()];
      final b = [for (var i = 0; i < 5; i++) LifeSimRng(2).nextU64()];
      // Realizat corect prin recreare, comparăm primele valori.
      final aa = LifeSimRng(1);
      final bb = LifeSimRng(2);
      final seqA = [for (var i = 0; i < 5; i++) aa.nextU64()];
      final seqB = [for (var i = 0; i < 5; i++) bb.nextU64()];
