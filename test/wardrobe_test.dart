...
void main() {
  ...
  group('catalog (guvernanța din spec)', () {
    test('every item has a fixed price XOR a merit condition; ids unique',
        () async {
      for (final locale in ['ro', 'en']) {
        final container = ProviderContainer();
        addTearDown(container.dispose);
        final catalog =
            await container.read(wardrobeCatalogProvider(locale).future);
        ...
        expect(merit, greaterThanOrEqualTo(3),
            reason: 'itemele câștigate prin merit sunt cârligul de retenție');
      }
    });

    test('daily showcase: deterministic, 3 buyable items, rounded discount',
        () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      final catalog =
          await container.read(wardrobeCatalogProvider('ro').future);
      ...
      expect(a.toSet(), isNot(dailyShowcase(catalog, '2026-07-13').toSet()),
          reason: 'vitrina se schimbă zilnic');
      final merits =
          catalog.where((i) => i.isMerit).map((i) => i.id).toSet();
  ...
  group('WardrobeRepository', () {
    ...
    test('purchase: needs funds, pays through the ledger, is idempotent',
        () async {
      expect(await repo.purchase(hat, showcased: false), isFalse,
      ...
      expect(ledger.any((e) => e.reason == 'wardrobe_acc_test' && e.delta == -200),
          isTrue);
    ...
    test('equip writes the profile slot; visit bonus pays once a day',
        () async {
