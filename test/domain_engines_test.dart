...
void main() {
  group('computeStreak', () {
    ...
    test('only today → 1/1', () {
      expect(
        computeStreak({'2026-07-07'}, '2026-07-07'),
        (current: 1, longest: 1),
      );
    });
    ...
    test('yesterday still holds the streak (Duolingo rule)', () {
      expect(
        computeStreak({'2026-07-05', '2026-07-06'}, '2026-07-07'),
        (current: 2, longest: 2),
      );
    });
    ...
    test('a 2-day gap breaks the current streak but longest survives', () {
      expect(
        computeStreak(
          {'2026-07-01', '2026-07-02', '2026-07-03', '2026-07-07'},
          '2026-07-07',
        ),
        (current: 1, longest: 3),
      );
    ...
    test('gap of exactly 2 days (activity day-before-yesterday) → 0 current',
        () {
      expect(
        computeStreak({'2026-07-05'}, '2026-07-07'),
        (current: 0, longest: 1),
      );
    });
  ...
  group('moodFor', () {
    test('no budget → happy', () {
      expect(moodFor(spentThisMonth: 900, monthlyBudget: null),
          CashyMood.happy);
      expect(
          moodFor(spentThisMonth: 900, monthlyBudget: 0), CashyMood.happy);
    });

    test('thresholds: 79.9% happy, 80% alert, 99.9% alert, 100% worried', () {
      expect(moodFor(spentThisMonth: 799, monthlyBudget: 1000),
          CashyMood.happy);
      expect(moodFor(spentThisMonth: 800, monthlyBudget: 1000),
          CashyMood.alert);
      expect(moodFor(spentThisMonth: 999, monthlyBudget: 1000),
          CashyMood.alert);
      expect(moodFor(spentThisMonth: 1000, monthlyBudget: 1000),
          CashyMood.worried);
      expect(moodFor(spentThisMonth: 1400, monthlyBudget: 1000),
          CashyMood.worried);
    });
  });
}
