...
void main() {
  ...
  group('evaluateStreak, break + earn-back', () {
    StreakSnapshot broken() {
      // 3-day streak, then a 3-day hole (> 2 freezes) → break.
      final r = evaluateStreak(
        snapshot: const StreakSnapshot(),
      ...
      expect(r.current, 1); // just today
      expect(r.snapshot.freezes, 2); // freezes NOT wasted on a lost cause
      return r.snapshot;
    }
    ...
    test('earn-back succeeds with 2 kinds inside the window', () {
      ...
      final r = evaluateStreak(
        snapshot: s,
        activityDays: {
          '2026-06-29', '2026-06-30', '2026-07-01', '2026-07-05'
        },
        todayKindCount: 2,
        today: '2026-07-05',
      );
      expect(r.events.whereType<EarnbackSucceeded>(), isNotEmpty);
      // Gap bridged: 29,30,1,(2,3,4 bridged),5 → 7 consecutive days.
      expect(r.current, 7);
      expect(r.snapshot.earnbackUntil, isNull);
    });

    test(
        'earn-back expires silently; the fresh gap is then freeze-protected '
        '(Duolingo semantics: freezes burn on any missed day)', () {
      final s = broken();
      final r = evaluateStreak(
        snapshot: s,
        activityDays: {
          '2026-06-29', '2026-06-30', '2026-07-01', '2026-07-05', '2026-07-08'
        },
        todayKindCount: 1,
        today: '2026-07-08',
      );
      ...
      // 05 (real) + 06,07 (frozen) + 08 (real) → 4; both freezes consumed.
      expect(r.current, 4);
      expect(r.snapshot.freezes, 0);
      expect(r.events.whereType<FreezeUsed>().length, 2);
    });
  ...
  group('quest engine', () {
    ...
    test('chest value: transparent floor grows with streak, bounded spread',
        () {
