...
void main() {
  group('pacing', () {
    ...
    test('under pace praises; near pace stays silent; gates hold', () {
      expect(
        _keys(evaluateInsights(_inputs(spent: 200))),
        contains('pace_under'),
      );
      expect(
        _keys(evaluateInsights(_inputs(spent: 380))),
        isNot(anyOf(contains('pace_over'), contains('pace_under'))),
      );
      // Sub 5 tranzacții sau fără buget: tăcere.
      expect(
        _keys(evaluateInsights(_inputs(spent: 600, txCount: 3))),
        isNot(contains('pace_over')),
      );
      expect(
        _keys(evaluateInsights(_inputs(spent: 600, budget: 0))),
        isNot(contains('pace_over')),
      );
    });
  });
  ...
  test('weekly recap fires only on Monday with enough data', () {
    ...
    expect(
      _keys(evaluateInsights(_inputs(thisWeek: 80, prevWeek: 120))),
  ...
  group('category anomaly vs own average', () {
    test('picks the worst offender, respects both thresholds', () {
      ...
      final anomaly = list.singleWhere((c) => c.ruleKey == 'category_anomaly');
    ...
    test('needs 2 full months of history; win fires after day 10', () {
      ...
      final win = evaluateInsights(
        _inputs(catMonth: {'fastfood': 20}, cat3m: {'fastfood': 100}),
  ...
  test('goal milestones report only the highest crossed threshold', () {
    ...
    expect(
      done.singleWhere((c) => c.id.startsWith('goal_')).ruleKey,
      'goal_done',
    );
  });
  ...
  test('fresh start on the 1st-2nd; saving milestone picks highest', () {
    ...
    final saving = evaluateInsights(
      _inputs(savedAllTime: 620),
