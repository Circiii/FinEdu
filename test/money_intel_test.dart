...
void main() {
  group('robustCenter / robustSpread (mediană + MAD)', () {
    ...
    test('spread: zero on constant data, scaled MAD otherwise', () {
      ...
      expect(robustSpread([15, 18, 20, 22, 25, 17]),
  ...
  group('detectRecurring', () {
    List<({String category, double amount, DateTime date})> series(
        String cat, List<DateTime> dates, List<double> amounts) {
      return [
    ...
    test('monthly Netflix-like series (30±2d, ±5% amount) is detected', () {
      final guesses = detectRecurring(series(
        'distractie',
        [
          DateTime(2026, 1, 5),
  ...
  group('categorySuggestion (naive Bayes cu abținere)', () {
    ...
    List<({String category, double amount, int weekday})> foodHistory(
        int n) {
      return [
        for (var i = 0; i < n; i++)
          (
    ...
    test('never throws on empty history or empty categories', () {
      expect(
          categorySuggestion(
              amount: 15, weekday: 2, history: const [], categories: cats),
          -1);
      expect(
          categorySuggestion(
              amount: 15,
  ...
  group('engine rules (F10-d)', () {
    ...
    test('safe_to_spend gates: budget, tx count, days left', () {
      expect(_keys(evaluateInsights(_inputs(budget: 0))),
          isNot(contains('safe_to_spend')));
      expect(_keys(evaluateInsights(_inputs(txCount: 4))),
          isNot(contains('safe_to_spend')));
    ...
    test('anomaly question fires on a fresh outlier expense', () {
      ...
      final q =
          list.singleWhere((c) => c.ruleKey == 'category_anomaly_question');
    ...
    test('radar skips declared guesses and falls to the next one', () {
      ...
      const fresh = RecurringGuess(
          ...
          confidence: 3);
      final list = evaluateInsights(_inputs(
          guesses: const [declared, fresh], declared: {'distractie'}));
      final radar = list.singleWhere((c) => c.ruleKey == 'recurring_radar');
      expect(radar.values['category'], 'transport');
    });
  });

  group('content: the F10-d insights.json entries', () {
    ...
    const emittedValues = {
      'safe_to_spend': {
        'safe', 'perDay', 'daysLeft', 'budget', 'spent', 'recurring', 'goals'
      },
      'category_anomaly_question': {'category', 'amount', 'median'},
      'recurring_radar': {'category', 'amount', 'period'},
    };

    test('entries exist, bilingual, on-tone, mojibake-free, placeholders ok',
        () {
      final root = jsonDecode(
              File('content/insights.json').readAsStringSync(encoding: utf8))
      ...
      // Lint de ton (F8-a): descrie datele, nu judeca persoana.
      ...
      emittedValues.forEach((key, allowed) {
        ...
        final variants =
            (rule['variants'] as List).cast<Map<String, dynamic>>();
        ...
        for (final s in texts) {
          expect(mojibake.hasMatch(s), isFalse,
