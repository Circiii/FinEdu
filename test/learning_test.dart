...
Lesson _lesson(String id, {int xp = 15}) => Lesson(
      ...
      action: 'A',
      cards: const [
        ConceptCard('c1', 'Q1', 'A1'),
        ConceptCard('c2', 'Q2', 'A2'),
      ],
    );

void main() {
  group('content', () {
    test('units parse in both locales; ids unique; interactives valid',
        () async {
      ...
      for (final locale in ['ro', 'en']) {
        ...
        expect([for (final u in units) u.ord],
            [for (var i = 1; i <= units.length; i++) i]);

        // Card ids must be unique ACROSS units: ReviewCards is keyed by
        // cardId with insertOrIgnore, so a duplicate would silently vanish.
        final lessonIds = <String>{};
        final cardIds = <String>{};
        for (final unit in units) {
          expect(['blue', 'green', 'amber', 'violet', 'danger'],
              contains(unit.color),
              reason: unit.id);
          String? previousKind;
          for (final lesson in unit.lessons) {
            ...
            for (final card in lesson.cards) {
              expect(cardIds.add(card.id), isTrue,
            ...
            switch (it.kind) {
              case 'mcq':
                expect(it.options.length, greaterThanOrEqualTo(2),
                    reason: where);
                expect(it.correct,
                ...
                if (unit.format >= 2) {
                  ...
                  for (var i = 0; i < it.options.length; i++) {
                    if (i != it.correct) {
                      expect(it.options[i].why, isNotNull,
              ...
              case 'swipe':
                expect(it.cards.length, greaterThanOrEqualTo(4),
                    reason: where);
              ...
              case 'poll':
                expect(it.pollOptions.length, greaterThanOrEqualTo(2),
                    reason: where);
              ...
              case 'cloze':
                ...
                expect(it.chips.length, greaterThanOrEqualTo(2),
                    reason: where);
                expect(it.correct,
              ...
              case 'order':
                ...
                expect(it.options.length, inInclusiveRange(3, 6),
                    reason: where);
              ...
              case 'pairs':
                expect(it.title, isNotNull, reason: where);
                expect(it.pairs.length, inInclusiveRange(3, 5), reason: where);
              ...
              case 'reveal':
                expect(it.title, isNotNull, reason: where);
                expect(it.reveals.length, inInclusiveRange(2, 4),
                    reason: where);
              ...
              case 'param_sim':
                ...
                for (final e in sim.exposed) {
                  expect(['years', 'monthly', 'rate'], contains(e),
                      reason: where);
                }
                expect(sim.rateDefault, inInclusiveRange(0, 8),
                    reason: '$where honest RO rates only');
                expect(sim.monthlyDefault, inInclusiveRange(0, 500),
                    reason: '$where teen-sized sums');
                expect(sim.yearsDefault, inInclusiveRange(1, sim.yearsMax),
                    reason: where);
            ...
            if (g != null) {
              expect(g.actual, inInclusiveRange(g.min, g.max),
                  reason: lesson.id);
              expect(g.step, greaterThan(0), reason: lesson.id);
              expect((g.max - g.min) % g.step, 0,
                  reason: '${lesson.id} slider divisions');
            }
            final check = lesson.check;
            if (check != null) {
              expect(check.options.length, inInclusiveRange(2, 3),
                  reason: lesson.id);
            ...
            if (sc != null) {
              expect(sc.options.length, inInclusiveRange(2, 4),
                  reason: lesson.id);
            ...
            if (unit.format >= 3) {
              ...
              expect(blocks, isNotNull, reason: '${lesson.id} missing blocks');
              expect(blocks!.length, inInclusiveRange(3, 7),
                  reason: '${lesson.id} block count');
              expect(blocks.whereType<TextBlock>().length,
              ...
              for (final b in blocks) {
                ...
                for (final t in texts) {
                  ...
                  expect('**'.allMatches(t).length.isEven, isTrue,
                      reason: '${lesson.id} unbalanced ** in "$t"');
                  expect('=='.allMatches(t).length.isEven, isTrue,
                      reason: '${lesson.id} unbalanced == in "$t"');
                }
                if (b is CalloutBlock) {
                  expect(['blue', 'amber', 'green', 'violet', 'danger'],
                      contains(b.tone),
                      reason: lesson.id);
                }
                if (b is StatBlock) {
                ...
                if (b is StepsBlock) {
                  expect(b.items.length, inInclusiveRange(2, 6),
                      reason: lesson.id);
                }
              }
            }
  ...
  group('LearnRepository', () {
    ...
    test('completeLesson: XP + acorns + cards enqueued + idempotent',
        () async {
      ...
      // Cards enqueued, due tomorrow (not today).
