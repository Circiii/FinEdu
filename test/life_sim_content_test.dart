...
void main() {
  ...
  test('bundle parses in both locales through the real engine parser', () {
    for (final locale in ['ro', 'en']) {
      final content = LifeSimContent.fromJsonBundle(
        loadBundle(),
        locale: locale,
      );
  ...
  test(
    'ids unique + references resolve (recurring, goals, events, chains)',
    () {
      ...
      for (final id in scheduled) {
        expect(
          eventIds,
          contains(id),
          reason: 'scheduleEvent -> $id inexistent',
        );
      }
      for (final e in c.events) {
    ...
    for (final locale in ['ro', 'en']) {
      ...
      for (final e in c.events) {
        if (e.narrative.length > 200) {
          problems.add(
            '[$locale] ${e.id}: narativ ${e.narrative.length} > 200',
          );
        }
