...
void main(List<String> args) {
  ...
  for (final f
      in Directory('content/life_sim/events').listSync().whereType<File>()) {
    bundle[f.path.replaceAll('\\', '/')] = f.readAsStringSync();
  }
  ...
  for (var i = 0; i < n; i++) {
    ...
    final policy = _policies[(i ~/ (roles.length * _modes.length)) %
        _policies.length];
    try {
      ...
      finalDebt.add(s.debts.fold<int>(
              0, (a, d) => a + d.principal.toJson()) ~/
          100);
  ...
  final buf = StringBuffer()
    ...
    ..writeln()
    ..writeln('> Generat determinist de tool/life_sim_monte_carlo.dart, '
        'N=$n run-uri, ${roles.length} roluri × ${_modes.length} moduri × '
        '${_policies.length} politici, contentVersion ${c.version}.')
    ..writeln()
    ..writeln('## Sinteză')
    ..writeln()
    ..writeln('- Run-uri terminate fără excepții: ${n - failures}/$n '
        '(${(100 * (n - failures) / n).toStringAsFixed(2)}%)')
    ..writeln('- Run-uri cu cash negativ la un moment dat: $cashNegativeEver '
        '(${(100 * cashNegativeEver / n).toStringAsFixed(1)}%)')
    ..writeln('- Dintre ele, recuperate (scor final >50): $recovered '
  ...
  for (final p in _policies) {
    ...
    buf.writeln('- **$p**: medie ${mean.toStringAsFixed(1)} · '
  ...
  buf
    ..writeln()
    ..writeln('## Dificultate pe roluri (media scorului total + cash final p50)')
    ..writeln();
  final globalMean = scoreByRole.values
          .expand((x) => x)
          .fold<int>(0, (a, b) => a + b) /
  ...
  for (final r in roles) {
    ...
    buf.writeln('- **$r**: scor mediu ${mean.toStringAsFixed(1)} · cash final '
  ...
  buf
    ...
    ..writeln('- Datorie finală p50: ${pct(finalDebt, 0.5)} lei · '
  ...
  for (final e in top.take(20)) {
    ...
    buf.writeln('- ${e.key}: ${e.value} apariții · în '
  ...
  stdout.writeln(buf);
  File('docs/research/life-month-balance-report.md')
...
int _pick(String policy, List<LifeChoice> choices, LifeSimRng rng) {
  int cashOf(LifeChoice ch) => ch.effects
      .whereType<CashDelta>()
      .fold<int>(0, (a, e) => a + e.delta.toJson());
  int statsOf(LifeChoice ch, {Set<String>? only}) => ch.effects
      .whereType<StatDelta>()
      .where((e) =>
