...
class LifeStats {
  ...
  factory LifeStats.initial() => const LifeStats(
        health: 75,
        energy: 75,
        stress: 25,
        relationships: 70,
  ...
  LifeStats withDelta(String stat, int delta) => switch (stat) {
        ...
        'relationships' =>
          copyWith(relationships: _clamp(relationships + delta)),
        _ => throw FormatException('stat necunoscut: $stat'),
      };
...
class DecisionRecord {
  ...
  Map<String, dynamic> toJson() =>
      {'day': day, 'eventId': eventId, 'choiceIdx': choiceIdx};
...
class LifeSimState {
  ...
  Money get totalDebt =>
      debts.fold(Money.zero, (sum, d) => sum + d.principal);
  ...
  Map<String, dynamic> toJson() => {
        ...
        if (lastEventDifficulty != null)
          'lastEventDifficulty': lastEventDifficulty,
  ...
  factory LifeSimState.fromJson(Map<String, dynamic> j) => LifeSimState(
        ...
        decisions: [
          for (final d
              in (j['decisions'] as List).cast<Map<String, dynamic>>())
            DecisionRecord.fromJson(d),
        ],
        categoryCounts:
            (j['categoryCounts'] as Map).map((k, v) => MapEntry(k as String, v as int)),
        missedBills: [
          for (final m
              in (j['missedBills'] as List).cast<Map<String, dynamic>>())
        ...
        arrears: j['arrears'] == null
            ...
            : [
                for (final a
                    in (j['arrears'] as List).cast<Map<String, dynamic>>())
        ...
        eventLastSeen: (j['eventLastSeen'] as Map)
            .map((k, v) => MapEntry(k as String, v as int)),
        firedEffects: [
          for (final f
              in (j['firedEffects'] as List).cast<Map<String, dynamic>>())
