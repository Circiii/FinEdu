...
class ClayIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: SvgIcon(path, size: iconSize, color: color, strokeWidth: strokeWidth, fill: fill),
    );
  }
}
      ...
      child: AnimatedContainer(
        ...
        child: Row(
          ...
          children: [
            ...
            Text(widget.label,
                style: T.display(size: widget.fontSize, weight: FontWeight.w800, color: widget.textColor)),
...
/// Donut ring gauge (budget). [percent] 0..1.
class RingGauge extends StatelessWidget {
  final double size;
  final double percent;
  final Gradient gradient;
  final double strokeWidth; // in viewBox(120) units
  final double radius; // in viewBox(120) units
    ...
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          CustomPaint(
            size: Size(size, size),
            painter: _RingPainter(percent, gradient, strokeWidth / 120 * size, radius / 120 * size, trackColor),
          ),
          ?center,
        ],
      ),
...
class _RingPainter extends CustomPainter {
  ...
  final double radius;
  final Color trackColor;
  _RingPainter(this.percent, this.gradient, this.strokeWidth, this.radius, this.trackColor);
...
/// Gauge speedometer 3/4 (scorul FinEdu). [percent] 0..1 din arcul de 270°.
class ScoreGauge extends StatelessWidget {
  final double width; // 86
  final double visibleHeight; // 62
  final double percent;
  final Widget? center;
      ...
      child: Stack(
        ...
        children: [
          CustomPaint(
            ...
            painter: _ScorePainter(percent, 22 / 220 * width, 90 / 220 * width),
          ),
          ?center,
        ],
      ),
...
class SegmentBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < segments.length; i++) {
      children.add(Expanded(flex: segments[i].$1, child: Container(color: segments[i].$2)));
        ...
        children: [
          Padding(
            ...
            child: Row(
              ...
              children: [
                Text('9:41', style: T.display(size: 15, weight: FontWeight.w700, color: C.text)),
                Row(
                  children: [
                    _signal(),
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: C.text.withValues(alpha: 0.5), width: 1.3),
              ),
            ),
            Positioned(
              left: 2,
              child: Container(
                width: 15,
                height: 7,
                decoration: BoxDecoration(color: C.blue, borderRadius: BorderRadius.circular(1.6)),
              ),
            ),
            Positioned(
              left: 23.4,
              child: Container(
                ...
                height: 4.2,
                decoration: BoxDecoration(
                    color: C.text.withValues(alpha: 0.5), borderRadius: BorderRadius.circular(1)),
              ),
            ),
          ],
        ),
...
class ClaySliderThumb extends SliderComponentShape {
  ...
  void paint(PaintingContext context, Offset center,
    ...
    canvas.drawShadow(Path()..addOval(Rect.fromCircle(center: center.translate(0, 3), radius: 13)),
        const Color(0xFF1560D8), 6, true);
    const grad = LinearGradient(
      begin: Alignment.topCenter, end: Alignment.bottomCenter,
      colors: [Color(0xFF7FB4FF), C.blue, C.blueDeep], stops: [0.0, 0.6, 1.0],
    );
    canvas.drawCircle(center, 15, Paint()..color = C.surface);
    canvas.drawCircle(center, 12, Paint()..shader = grad.createShader(Rect.fromCircle(center: center, radius: 12)));
  }
}
...
class BottomNav extends StatelessWidget {
    ...
    return Stack(
      ...
      children: [
        ...
        Transform.translate(
          ...
          child: GestureDetector(onTap: onFab, child: _fab()),
        ),
      ],
    );
  }
          ...
          children: [
            if (isActive)
              Container(
                width: 42,
                height: 32,
                decoration: BoxDecoration(color: activeBg, borderRadius: BorderRadius.circular(11)),
                alignment: Alignment.center,
                child: SvgIcon(item.path, size: 22, color: color, strokeWidth: 2.1),
              )
            else
              SvgIcon(item.path, size: 22, color: color, strokeWidth: 2),
            SizedBox(height: isActive ? 2 : 4),
            Text(item.label,
                style: T.display(size: 10, weight: isActive ? FontWeight.w800 : FontWeight.w700, color: color)),
          ],
        ),
      ),
    );
  ...
  Widget _fab() => Container(
        ...
        alignment: Alignment.center,
        child: const SvgIcon(Ic.plus, size: 28, color: Colors.white, strokeWidth: 2.8),
      );
}
