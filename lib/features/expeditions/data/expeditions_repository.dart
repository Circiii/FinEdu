...
final postcardsProvider =
    FutureProvider.family<List<String>, String>((ref, locale) async {
  final json = jsonDecode(await loadAssetString('content/expeditions.json'))
...
class ExpeditionsRepository {
  ...
  Stream<ExpeditionRow?> watchToday() {
    ...
    return (_db.select(_db.expeditionRows)..where((e) => e.day.equals(today)))
        .watchSingleOrNull();
  }
  ...
  Future<void> depart({required int streak}) async {
    final today = dayKey(DateTime.now());
    await _db.into(_db.expeditionRows).insert(
  ...
  Future<int> collect() async {
    final today = dayKey(DateTime.now());
    final row = await (_db.select(_db.expeditionRows)
          ..where((e) => e.day.equals(today)))
        .getSingleOrNull();
    if (row == null || row.collectedAt != null) return 0;
    if (DateTime.now().difference(row.departedAt) < _duration) return 0;

    final updated = await (_db.update(_db.expeditionRows)
  ...
  Future<int> autoCollectStale() async {
    ...
    final rows = await (_db.select(_db.expeditionRows)
          ..where((e) => e.collectedAt.isNull() & e.day.isNotValue(today)))
        .get();
    var total = 0;
